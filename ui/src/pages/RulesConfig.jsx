  ...
  return (
    <div className="gs-rules">
      ...
      <div className="gs-rules-tabs">
        ...
        <button className={`gs-rules-tab-btn ${activeTab === "guide" ? "active" : ""}`} onClick={() => { setActiveTab("guide"); setActionResult(null); }}>
          📖 Configuration Guide
        </button>
      </div>
      ...
      {activeTab === "guide" && (
        <div style={{ maxWidth: '960px', margin: '20px auto 0', display: 'flex', flexDirection: 'column', gap: '20px' }}>

          {/* === HEADER === */}
          <div className="gs-rcard" style={{ background: 'linear-gradient(135deg, var(--accent-purple) 0%, #7c3aed 100%)', color: 'white', padding: '28px 32px' }}>
          ...
          {/* === SECTION 6: Semi-Auto ETL Decision Framework === */}
          <div className="gs-rcard">
            <h3 style={{ fontSize: '15px', fontWeight: 800, color: 'var(--accent-purple)', marginBottom: '16px', borderBottom: '2px solid var(--accent-purple)', paddingBottom: '8px' }}>
              🧩 Semi-Auto ETL Decision Framework
            </h3>
            <p style={{ fontSize: '11.5px', color: 'var(--text-secondary)', marginBottom: '16px', lineHeight: 1.5 }}>
              How System Assists User — กระบวนการทำงาน 6 ขั้นตอน
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {[
                { step: 1, title: 'Data Purpose', desc: 'กำหนดวัตถุประสงค์ของข้อมูล เช่น Official, Critical หรือ Exploratory', icon: '🎯', bg: 'rgba(139,92,246,0.06)' },
                { step: 2, title: 'Data Profiling', desc: 'ระบบสแกนหา Null, Outlier และ Schema โดยอัตโนมัติ', icon: '🔍', bg: 'rgba(59,130,246,0.06)' },
                { step: 3, title: 'Business Rules', desc: 'ระบุเงื่อนไขบังคับ เช่น ค่าใดผิด หรือค่าใดห้ามหาย', icon: '📋', bg: 'rgba(245,158,11,0.06)' },
                { step: 4, title: 'Platform Guide', desc: 'ระบบเสนอ Starting Parameter Range ที่เหมาะสมให้ผู้ใช้', icon: '🤖', bg: 'rgba(139,92,246,0.06)' },
                { step: 5, title: 'User Reviews', desc: 'คนเข้าตรวจสอบและปรับจูนตาม SLA และระดับความเสี่ยง (Risk)', icon: '👤', bg: 'rgba(16,185,129,0.06)' },
                { step: 6, title: 'Transform', desc: 'กดยืนยันเพื่อรัน Pipeline สะอาดโดยอัตโนมัติ', icon: '✅', bg: '#d1fae5' }
              ].map((item) => (
                <div key={item.step} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '12px 16px', background: item.bg, borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                  <div style={{ background: 'var(--accent-purple)', color: 'white', borderRadius: '50%', minWidth: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 800 }}>{item.step}</div>
                  <div style={{ flex: 1 }}>
                    <span style={{ fontSize: '12px', fontWeight: 700 }}>{item.icon} {item.title}</span>
                    <span style={{ fontSize: '11px', color: 'var(--text-secondary)', marginLeft: '8px' }}>{item.desc}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* Key Takeaway */}
            <div style={{ marginTop: '16px', padding: '14px 18px', background: 'linear-gradient(135deg, rgba(139,92,246,0.08), rgba(59,130,246,0.08))', border: '1px solid rgba(139,92,246,0.2)', borderRadius: '10px' }}>
              <div style={{ fontSize: '12px', fontWeight: 800, color: 'var(--accent-purple)', marginBottom: '6px' }}>💡 Key Takeaway (Semi-Auto Architecture)</div>
              <div style={{ fontSize: '11.5px', lineHeight: 1.7, color: 'var(--text-secondary)' }}>
                แพลตฟอร์มไม่ได้ให้ผู้ใช้มานั่งเดาพารามิเตอร์เอง แต่จะ<strong>วิเคราะห์ข้อมูล</strong> นำ<strong>วัตถุประสงค์และกฎทางธุรกิจ</strong>มาพิจารณา
                จากนั้นจึง<strong>แนะนำช่วงค่าเริ่มต้นที่สมเหตุสมผล</strong>เพื่อให้ผู้ใช้ตรวจสอบและยืนยันก่อนใช้งานจริง
              </div>
            </div>
          </div>
